...
lengthList = [ 6, 8, 10, 12, 14, 16, 18]
cols = len(lengthList)
testAmount = 30
recordedTimesNaive = [[0]*cols]*testAmount
...
for i in range(30):
    ...
    for idxOfList in range(len(lengthList)):
        ...
        startTime = time.time()
        lcs(randomSequence1, randomSequence2, lX, lY)
        endTime = time.time()
        recordedTimesNaive[i][idxOfList] = endTime-startTime
        print("Naive with m=n="+str(lengthList[idxOfList])+" =>",endTime-startTime)
...
print("Naive mean/std:")
for n in range(len(lengthList)):
    mean = 0
    for i in range(testAmount):
        mean += recordedTimesNaive[i][n]
    mean = mean / len(lengthList)
    naiveMeanList.append(mean)
    meanDiff = 0
    for i in range(testAmount):
        meanDiff += pow((mean-recordedTimesNaive[i][n]),2)
    std = math.sqrt(meanDiff/(len(lengthList)-1))
    naiveStdList.append(std)
...
plt.plot(lengthList,naiveMeanList,linewidth=2.0)
plt.xlabel('input size')
plt.ylabel('runtime (recursive)')
plt.show()
