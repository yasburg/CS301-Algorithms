...
if __name__ == '__main__':
    ...
    for i in numOfCityList:
        ...
        endtime = time.time()
        algoTimeNaive = endtime-starttime
        algoTimesNaive.append(algoTimeNaive)

        starttime = time.time()
        print("Total minimum cost:", shortestPathFinder(costMatrix, path, numOfStations, startStation, targetStations))
        endtime = time.time()
        algoTimeDP = endtime-starttime
        algoTimesDP.append(algoTimeDP)


    plt.plot(numOfCityList, algoTimesNaive, linewidth=1.0)
    plt.plot(numOfCityList, algoTimesDP, linewidth=1.0)
